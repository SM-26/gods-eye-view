...
`.gev-cache` and `.gev-logs` live on the host and source edits still hot-reload.
The container binds `0.0.0.0` *inside* the container, but the port is published
to `127.0.0.1` only, so the default matches the native one: nobody else can
reach your server. Widening that mapping puts you under
